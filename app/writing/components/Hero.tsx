...
const ArticleTags = React.memo<{ tags: ReadonlyArray<string> | string[] | undefined }>(({ tags }) => {
...
const PrimaryArticle = React.memo<{ article: Article }>(({ article }) => (
  <article className="lg:col-span-7">
    <Link href={article.link} className="block focus-visible:outline-accent focus-visible:outline-2 focus-visible:outline-offset-2 rounded-xl">
      <div className="rounded-xl overflow-hidden shadow-xl border-l-4" style={{ borderColor: 'var(--color-white)' }}>
        ...
        <div className="mt-4 p-6 flex flex-col">
          ...
          <p className="mt-2 text-[var(--color-text-secondary)] line-clamp-2">{article.description}</p>
        </div>
      </div>
    </Link>
  </article>
  ...
  <article role="listitem" className="h-32">
    <Link href={article.link} className="flex gap-3 p-2 rounded-lg">
      ...
      <div className="flex-1 min-w-0">
        <h3 className="font-medium text-[var(--color-text-primary)] line-clamp-2">{article.title}</h3>
...
const Hero = React.memo<HeroProps>(({ primaryArticle, featuredArticles, onRefresh }) => (
  <section className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 pt-6 sm:pt-8" aria-label="Featured Articles">

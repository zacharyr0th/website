...
const Hero = ({ primaryArticle, featuredArticles, onRefresh }: HeroProps) => (
  <section className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8" aria-label="Featured Articles">
...
const PrimaryArticle = ({ article }: { article: Article }) => (
  <article className="lg:col-span-7">
    ...
    >
      <div className="rounded-xl overflow-hidden bg-gradient-to-b from-var(--color-surface) to-var(--color-background) shadow-xl border-l-4 border-var(--color-accent)">
...
const Tags = ({ tags }: { tags: readonly string[] | undefined }) => (
  ((tags ? [...tags] : []).length > 0) && (
    <div className="flex flex-wrap gap-2 mb-3" role="list" aria-label="Article tags">
      {(tags ? [...tags] : []).map((tag) => (
        <span
          key={tag}
          role="listitem"
          className="px-2 py-1 text-xs font-medium rounded-full bg-var(--color-accent)/10 text-var(--color-accent)"
        >
          {tag}
        </span>
      ))}
...
const RandomSelection = ({ articles, onRefresh }: { articles: Article[]; onRefresh: () => void }) => (

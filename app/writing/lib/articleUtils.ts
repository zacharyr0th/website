...
export const getArticleFiles = async () => {
  ...
  try {
    ...
    return fs.default.readdirSync(articlesDirectory)
 ...
 */
export const processFrontmatter = (rawFrontmatter: RawFrontmatter, slug: string): ArticleFrontmatter => {
  return {
    title: rawFrontmatter.title.substring(0, ARTICLE_CONFIG.limits.title),
    date: rawFrontmatter.date || new Date().toISOString(),
    description: (rawFrontmatter.description || rawFrontmatter.subtitle || '')
      .substring(0, ARTICLE_CONFIG.limits.description),
    category: (rawFrontmatter.category as ArticleCategory) || null,
    tags: (rawFrontmatter.tags as ArticleTag[]) || [],
    image: typeof rawFrontmatter.image === 'string'
...
export const parseArticleFile = async (fileName: string): Promise<Article | null> => {
  ...
  try {
    ...
    const frontmatter = processFrontmatter(data as RawFrontmatter, slug);
